...
/* used with the PmodTMP2 IP core.                                            */
/*                                                                            */
/******************************************************************************/
/* Revision History:                                                          */
/*                                                                            */
...
**
**   Parameters:
**      InstancePtr:  A PmodTMP2 object to start
**      IIC_Address:  The Base address of the PmodTMP2 IIC
...
**
**   Description:
**      Initialize the PmodTMP2.
*/
void TMP2_begin(PmodTMP2 *InstancePtr, u32 IIC_Address, u8 Chip_Address) {
   TMP2_Config.BaseAddress = IIC_Address;
   InstancePtr->chipAddr = Chip_Address;
   TMP2_IICInit(&InstancePtr->TMP2Iic);
   XIic_SetAddress(&InstancePtr->TMP2Iic, XII_ADDR_TO_SEND_TYPE,
...
**
**   Parameters:
**      InstancePtr: PmodTMP2 object to stop
**
**   Return Value:
**      none
**
...
**
**   Description:
**      Initializes the PmodTMP2 IIC.
*/

int TMP2_IICInit(XIic *IicInstancePtr) {
   int Status;

   Status = XIic_CfgInitialize(IicInstancePtr, &TMP2_Config,
         TMP2_Config.BaseAddress);
...
**
**   Parameters:
**      InstancePtr: PmodTMP2 object to initialize
...
void TMP2_ReadIIC(PmodTMP2 *InstancePtr, u8 reg, u8 *Data, int nData) {
   int Status;
   Status = XIic_Start(&InstancePtr->TMP2Iic);
   if (Status != XST_SUCCESS) {
      return;
   }
   if (InstancePtr->currentRegister != reg) {

      XIic_Send(InstancePtr->TMP2Iic.BaseAddress, InstancePtr->chipAddr, &reg,
            1, XII_REPEATED_START_OPTION);
      InstancePtr->currentRegister = reg;
   }
   XIic_Recv(InstancePtr->TMP2Iic.BaseAddress, InstancePtr->chipAddr, Data,
         nData, XIIC_STOP);

   Status = XIic_Stop(&InstancePtr->TMP2Iic);
   if (Status != XST_SUCCESS) {
      return;
   }
}
...
void TMP2_WriteIIC(PmodTMP2 *InstancePtr, u8 reg, u8 *Data, int nData) {
   ...
   XIic_Send(InstancePtr->TMP2Iic.BaseAddress, InstancePtr->chipAddr, out,
         nData + 1, XIIC_STOP);
...
**   Parameters:
**      InstancePtr:   A PmodTMP2 object to start
**      configuration: 8 bit TMP2 configuration code, refer to "Configuration
...
**
**   Description:
**      Configures the PmodTMP2's Microchip TCN75A. Does not need to be called on startup.
*/
void TMP2_config(PmodTMP2 *InstancePtr, u8 configuration) {
   u8 buf[1];
   buf[0] = configuration;
   TMP2_WriteIIC(InstancePtr, TMP2_REG_CONFIG, buf, 1);
}
...
**
**   Return Value:
**      tempResult: Current temperature seen by the TMP2 in degrees Celsius.
**
**   Errors:
**      none
**
**   Description:
**      Requests and returns the temperature off of the PmodTMP2
*/
double TMP2_getTemp(PmodTMP2 *InstancePtr) {
   double tempResult;
   u16 uResult = 0;
   ...
   TMP2_ReadIIC(InstancePtr, TMP2_REG_TEMP, buf, 2);
